...
"""
features_path = '../features/data_nmf_good_charword.pkl'
data_fp = open(features_path, 'rb')
x_train, y_train, x_test = pickle.load(data_fp)
data_fp.close()

"""划分训练集和验证集，验证集比例为test_size"""
x_train, x_vali, y_train, y_vali = train_test_split(x_train[:10000], y_train[:10000], test_size=0.1, random_state=0)
...
bst = lgb.train(params, d_train, num_boost_round=2000, valid_sets=d_vali,feval=f1_score_vali, early_stopping_rounds=None,
                verbose_eval=True)
...
"""
y_test = np.argmax(bst.predict(x_test[:10000]), axis=1) + 1
